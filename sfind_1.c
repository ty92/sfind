...
void do_find_argu(); //function declaration
char *find_result();

find_file(char *option[], struct find_options *x, int arg_num)
{
	DIR *dir_ptr;
	...
	else
	{
		...
		{
			if(strcmp(direntp->d_name,"..") == 0) //不对".."进行判断
				continue;
				
			if(stat(direntp->d_name, &info) == -1)
				perror("stat");
			do_find_argu(direntp->d_name, option, x, &info, arg_num);
		}
		closedir(dir_ptr);
	}
}

void do_find_argu(char *name, char *option[], struct find_options *x, struct stat *info, int arg_num)
{
	char *filename;
	int i=0, m=0;
	...
	if(arg_num==1)        //一个参数时
		...
		while(i<5)
                ...
                if(m==arg_num)        //一个文件筛选出的个数满足参数个数n，即该文件匹配所有选项，输出
                        printf("./%s\n",filename);
                m=0;
        }
}
